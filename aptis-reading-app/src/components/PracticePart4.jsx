...
const PracticePart4 = () => {
  ...
  const { weakIds, slowIds, masteredIds, prefilledStatuses } = useMemo(() => {
    ...
    const stats = {};
    const sortedHistory = [...history].filter(h => h.part === 'part-4').sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  const activeQuestions = useMemo(() => {
    ...
    if (isWeak || isSlow) {
      ...
      if (list.length === 0) list = [...part4Data];
    }
  ...
  useEffect(() => {
    ...
    if (currentQuestion) {
      // We need to shuffle them for display
      const headingsWithIndex = currentQuestion.headings.map((h, i) => ({ text: h, originalIndex: i }));
      // Fisher-Yates
      for (let i = headingsWithIndex.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [headingsWithIndex[i], headingsWithIndex[j]] = [headingsWithIndex[j], headingsWithIndex[i]];
      }
      setShuffledHeadings(headingsWithIndex);
      
      if (isStudy) {
        const correctAnswers = {};
        currentQuestion.paragraphs.forEach((_, idx) => {
          correctAnswers[idx] = idx; // Correct answer is always the same index as the paragraph
        });
  ...
  useEffect(() => {
    let timer;
    if (currentQuestion && isChecked && score === currentQuestion.paragraphs.length && !isStudy) {
      timer = setTimeout(() => {
  ...
  const handleCheck = () => {
    ...
    setQuestionStatuses(prev => ({
      ...prev,
      [currentQuestion.id]: currentScore === currentQuestion.paragraphs.length ? 'correct' : 'incorrect'
    }));
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', paddingBottom: '3rem' }}>
      ...
      <div className="practice-header" style={{ alignItems: 'center' }}>
        <div>
          ...
          <p className="text-muted">
            Set {part4Data.findIndex(q => q.id === currentQuestion.id) + 1} of {part4Data.length}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
              <label title="Random" style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', justifyContent: 'center', minWidth: '40px', height: '32px', cursor: 'pointer', borderRadius: 'var(--radius-md)', background: settings.randomizeQuestions ? 'rgba(59, 130, 246, 0.1)' : 'var(--bg-card)', color: settings.randomizeQuestions ? 'var(--primary)' : 'var(--text-muted)', border: `1px solid ${settings.randomizeQuestions ? 'var(--primary)' : 'var(--border)'}`, transition: 'all 0.2s', padding: '0 0.5rem' }}>
                ...
                />
                <Shuffle size={14} />
                <span style={{ fontSize: '0.75rem', fontWeight: 600 }}>{part4Data.length}</span>
              </label>
      ...
      <div className="question-board">
        {part4Data.map((origQ, originalIndex) => {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginTop: '1rem' }}>
        ...
        <div className="practice-board glass" style={{ padding: '0', maxHeight: '70vh', overflowY: 'auto' }}>
          <div style={{ padding: '2rem', background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)' }}>
            <h3 style={{ marginBottom: '1.5rem', color: 'var(--primary)', borderBottom: '2px solid var(--primary)', paddingBottom: '0.5rem', display: 'inline-block' }}>Đoạn văn (Passage)</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              {currentQuestion.paragraphs.map((para, idx) => (
                <div key={idx} style={{ position: 'relative' }}>
                  <div style={{ position: 'absolute', top: 0, left: '-2.5rem', fontWeight: 'bold', fontSize: '1.2rem', color: 'var(--text-muted)' }}>
                    {idx + 1}.
                  </div>
                  <p style={{ fontSize: '1.1rem', lineHeight: '1.8', color: 'var(--text-main)', textAlign: 'justify', whiteSpace: 'pre-wrap' }}>
                    {para}
                  </p>
                </div>
              ))}
        ...
        {/* Right Column: Questions & Selectors */}
        <div className="practice-board glass" style={{ padding: '0', display: 'flex', flexDirection: 'column', maxHeight: '70vh' }}>
          <div style={{ padding: '2rem', flex: 1, overflowY: 'auto' }}>
            <h3 style={{ marginBottom: '1.5rem', color: 'var(--primary)', borderBottom: '2px solid var(--primary)', paddingBottom: '0.5rem', display: 'inline-block' }}>Chọn Tiêu Đề (Headings)</h3>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              {currentQuestion.paragraphs.map((_, pIdx) => {
                const isSelectedCorrect = isChecked && selectedAnswers[pIdx] === pIdx;
                const isSelectedWrong = isChecked && selectedAnswers[pIdx] !== undefined && selectedAnswers[pIdx] !== pIdx;
                  ...
                  <div key={pIdx} style={{ 
                    ...
                    {isSelectedWrong && (
                      <div style={{ marginTop: '0.75rem', padding: '0.75rem', background: 'rgba(239,68,68,0.1)', color: 'var(--danger)', borderRadius: 'var(--radius-sm)', fontSize: '0.9rem' }}>
                        <strong>Sai!</strong> Đáp án đúng là: <em>{currentQuestion.headings[pIdx]}</em>
                      </div>
                    )}
                  </div>
                );
          ...
          <div className="board-footer" style={{ padding: '1.5rem 2rem', borderTop: '1px solid var(--border)', background: 'var(--bg-card)', borderRadius: '0 0 var(--radius-lg) var(--radius-lg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              {!isStudy && (
                <button 
                  onClick={handleCheck} 
                  disabled={isChecked || Object.keys(selectedAnswers).length < currentQuestion.paragraphs.length} 
